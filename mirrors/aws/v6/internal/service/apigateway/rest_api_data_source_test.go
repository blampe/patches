...
func TestAccAPIGatewayRestAPIDataSource_Endpoint_ipAddressType(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRestAPIDataSourceConfig_Endpoint_ipAddressType(rName, "REGIONAL", "dualstack"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPair(dataSourceName, names.AttrName, resourceName, names.AttrName),
					resource.TestCheckResourceAttrPair(dataSourceName, names.AttrARN, resourceName, names.AttrARN),
					resource.TestCheckResourceAttrPair(dataSourceName, "root_resource_id", resourceName, "root_resource_id"),
					...
					resource.TestCheckResourceAttrPair(dataSourceName, "binary_media_types", resourceName, "binary_media_types"),
					resource.TestCheckResourceAttrPair(dataSourceName, "endpoint_configuration.#", resourceName, "endpoint_configuration.#"),
					resource.TestCheckResourceAttrPair(dataSourceName, "endpoint_configuration.0.ip_address_type", resourceName, "endpoint_configuration.0.ip_address_type"),
					resource.TestCheckResourceAttrPair(dataSourceName, "execution_arn", resourceName, "execution_arn"),
				),
			},
		},
...
func testAccRestAPIDataSourceConfig_Endpoint_ipAddressType(rName, endpointType, ipAddressType string) string {
	return acctest.ConfigCompose(
		testAccRestAPIConfig_ipAddressType(rName, endpointType, ipAddressType),
		`
data "aws_api_gateway_rest_api" "test" {
  name = aws_api_gateway_rest_api.test.name
}
