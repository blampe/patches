...
const (
	packageName = "notificationscontacts"
	awsEnvVar   = "AWS_ENDPOINT_URL_NOTIFICATIONSCONTACTS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "notificationscontacts"
)
...
func defaultEndpoint(region string) (url.URL, error) {
	r := notificationscontacts.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), notificationscontacts.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.NotificationsContactsClient(ctx)

	var result apiCallParams

	input := notificationscontacts.ListEmailContactsInput{}
	_, err := client.ListEmailContacts(ctx, &input,
		func(opts *notificationscontacts.Options) {
