...
const (
	packageName = "observabilityadmin"
	awsEnvVar   = "AWS_ENDPOINT_URL_OBSERVABILITYADMIN"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "observabilityadmin"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := observabilityadmin.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, observabilityadmin.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ObservabilityAdminClient(ctx)

	var result apiCallParams

	input := observabilityadmin.ListCentralizationRulesForOrganizationInput{}
	_, err := client.ListCentralizationRulesForOrganization(ctx, &input,
		func(opts *observabilityadmin.Options) {
