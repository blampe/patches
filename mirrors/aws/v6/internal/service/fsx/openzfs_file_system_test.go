...
import (
	...
	tffsx "github.com/blampe/patches/mirrors/aws/v6/internal/service/fsx"
...
func TestAccFSxOpenZFSFileSystem_intelligentTiering(t *testing.T) {
	ctx := acctest.Context(t)
	var filesystem awstypes.FileSystem
	resourceName := "aws_fsx_openzfs_file_system.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccOpenZFSFileSystemConfig_intelligentTiering(rName),
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "read_cache_configuration.0.size", "64"),
					resource.TestCheckResourceAttr(resourceName, "read_cache_configuration.0.sizing_mode", string(awstypes.OpenZFSReadCacheSizingModeUserProvisioned)),
					resource.TestCheckResourceAttr(resourceName, "root_volume_configuration.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "root_volume_configuration.0.data_compression_type", "NONE"),
					resource.TestCheckResourceAttr(resourceName, "root_volume_configuration.0.nfs_exports.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "root_volume_configuration.0.nfs_exports.0.client_configurations.#", "1"),
...
func testAccOpenZFSFileSystemConfig_intelligentTiering(rName string) string {
	return acctest.ConfigCompose(testAccOpenZFSFileSystemConfig_baseMultiAZ(rName), fmt.Sprintf(`
resource "aws_fsx_openzfs_file_system" "test" {
  skip_final_backup   = true
  subnet_ids          = aws_subnet.test[*].id
