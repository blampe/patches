...
const (
	packageName = "odb"
	awsEnvVar   = "AWS_ENDPOINT_URL_ODB"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := odb.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, odb.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ODBClient(ctx)

	var result apiCallParams

	input := odb.ListCloudExadataInfrastructuresInput{}
	_, err := client.ListCloudExadataInfrastructures(ctx, &input,
		func(opts *odb.Options) {
