...
const (
	packageName = "s3vectors"
	awsEnvVar   = "AWS_ENDPOINT_URL_S3VECTORS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "s3vectors"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := s3vectors.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, s3vectors.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.S3VectorsClient(ctx)

	var result apiCallParams

	input := s3vectors.ListVectorBucketsInput{}
	_, err := client.ListVectorBuckets(ctx, &input,
		func(opts *s3vectors.Options) {
