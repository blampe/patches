	...
	acctest.Test(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				...
				ConfigStateChecks: []statecheck.StateCheck{
					tfstatecheck.ExpectRegionalARNAlternateRegionFormat(resourceName, tfjsonpath.New(names.AttrARN), "codeartifact", "domain/{domain}"),
					statecheck.CompareValuePairs(resourceName, tfjsonpath.New(names.AttrID), resourceName, tfjsonpath.New(names.AttrARN), compare.ValuesSame()),
...
func testAccCodeArtifactDomain_Identity_ExistingResource(t *testing.T) {
	...
	acctest.Test(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ConfigDirectory: config.StaticDirectory("testdata/Domain/basic_v5.100.0/"),
				ConfigVariables: config.Variables{
					acctest.CtRName: config.StringVariable(rName),
				},
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			{
				ConfigDirectory: config.StaticDirectory("testdata/Domain/basic_v6.0.0/"),
				ConfigVariables: config.Variables{
					acctest.CtRName: config.StringVariable(rName),
				},
				Check: resource.ComposeAggregateTestCheckFunc(
