...
const (
	packageName = "bedrockagentcore"
	awsEnvVar   = "AWS_ENDPOINT_URL_BEDROCK_AGENTCORE_CONTROL"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "bedrock_agentcore_control"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := bedrockagentcorecontrol.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, bedrockagentcorecontrol.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.BedrockAgentCoreClient(ctx)
